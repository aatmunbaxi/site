...
:EXPORT_FILE_NAME: ~/websites/site/public/index
:END:
...
:EXPORT_HUGO_MENU: :menu main
:END:
** How Emacs Helped Me Today: modus-themes and ef-themes Palettes for LaTeX :emacs:lisp:LaTeX:
:PROPERTIES:
:EXPORT_HUGO_SECTION: comp
:EXPORT_FILE_NAME: how_emacs_helped_modus_ef_palette_beamer
:END:
...
After manually reading off some colors from =modus-operandi-tinted-palette= into my LaTeX preamble, I remembered: /Wait, this is emacs. There's a better way to do this./
...
Defining the beamer theme is then a matter of using =\setbeamercolor= commands and change the colors of the desired presentation elements using a guide such as [[https://www.cpt.univ-mrs.fr/~masson/latex/Beamer-appearance-cheat-sheet.pdf][this one.]]
...
** DRAFT Avy-backed, laser-precise marking in Emacs
/TL;DR: We define a minor mode =lasgun= which uses =avy= to place inactive marks in emacs buffers./

Before reading this, it's worth reading Karthik's very comprehensive [[https://karthinks.com/software/avy-can-do-anything/#fc6ea2][article]] on the emacs package =avy=.
To vastly oversimplify things, =avy= allows tree-based selection and action on virtually any position in a visible buffer (and/or visible window).
** DRAFT Consistent, Dynamic Access to Custom Emacs Theme Palettes with technicolor
...
Sometimes I have a modus or ef theme, sometimes catppuccin.
This presents a challenge for what I want to accomplish, which we'll discuss an small example of.
...
- TODO :: insert video of switching theme on verbatim text

Blegh!
The code accesses the variable =doom-themes--colors=, which remains unchanged upon loading a non-DOOM theme.
The green from the previous DOOM theme palette sticks out as just /wrong/ here.
How can we fix this?

Not all hope is lost; the modus theme pack also comes with a function you can use to access color values from the current modus theme: =modus-themes-get-color-value=.
Huzzah!
...
#+begin_src emacs-lisp
  (set-face-attribute org-verbatim t :foreground (current-theme-color 'green))
#+end_src
...
In conclusion, there's no reasonable way to uniformly identify arbitrary colors in two different loaded themes, short of a universal standard™ ([[https://xkcd.com/927/][obligatory]]) for how to define custom emacs themes.
...
This is the design philosophy of [[https://github.com/aatmunbaxi/technicolor][technicolor]].

In our situation, we want to access the foreground color of DOOM themes, modus themes, and catppuccin themes.
For good measure, we'll throw in background, red, green, blue, magenta, and cyan into that list.
...
The standard function used to refer to a color in the current theme's palette is =technicolor-get-color=.
The argument for this function will be =color=, which is a symbol in =technicolor-colors=.
...
Notice it isn't a key in any of the above alists, since all our themes use the symbol =red= to identify the color green.
Similarly for =blue=.

What does this abstraction buy us?O
Our original face customization now looks like this:
#+begin_src emacs-lisp
  (set-face-attribute org-verbatim t :foreground (technicolor-get-color 'green))
#+end_src
This now works seamlessly across themes that are matched in =technicolor-themes=.

- TODO :: video showing correct transitions
* Misc
:PROPERTIES:
