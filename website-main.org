...
Kitaev [[cite:&kitaevAnyonsExactlySolved2006]] posited that the algebraic combinatorics of the solutions to these equations can be formalized in the language of *unitary modular categories*.
...
The reader might ask if the "braiding" in Figure [[fig:modular_definition_tree]] is related to the braiding discussed for anyons.
Indeed they are, the braiding of a unitary modular category corresponds to braiding of anyons.
There is an expansive "dictionary" of the correspondence between unitary modular categories and the physical interpretation of what the data represent.
This table can be found in more detail in [[cite:&rowellMathematicsTopologicalQuantum2018]].
...
#+caption: Synonyms from the unitary modular category realm and anyon system world
...
For a proof via nice exercises, see [[cite:&etingofTensorCategories2016]].
