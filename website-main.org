...
** \( \operatorname{Rep}(G) \)-type Fusion Rules                  :categories:
:PROPERTIES:
:EXPORT_HUGO_SECTION: math
...
:EXPORT_HUGO_MENU: :menu main
:END:
** Configuring =meow= for Friendly LaTeX Editing :emacs:meow:
:PROPERTIES:
:EXPORT_HUGO_SECTION: comp
...
Alas, I feel modal editing is something I prefer, so I've adopted to =meow=.
