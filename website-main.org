...
Unfortunately, my elisp abilities aren't good enough to handle the unmodified delimiters as well as the modified delimiters, so we only implement a thing for modified delimiters.
Let's start be declaring a master list of all possible modified delimiters we might find in a LaTeX file.
#+begin_src emacs-lisp
